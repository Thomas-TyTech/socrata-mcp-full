    ...
    handler: async (args, extra) => {
        ...
        try {
            ...
            if (attribution !== undefined)
                updatedMetadata.attribution = attribution;
            if (license !== undefined)
                updatedMetadata.license = license;
...
export async function getAssetMetadata(domain, assetId) {
    validateDomain(domain);
    const url = `https://${domain}/api/views/${assetId}.json`;
    return await socrataGet(url);
}
export async function getAssetPermissions(domain, assetId) {
    validateDomain(domain);
    const url = `https://${domain}/api/assets/${assetId}/permissions.json`;
    return await socrataGet(url);
}
export async function updateAssetPermissions(domain, assetId, updates) {
    validateDomain(domain);
