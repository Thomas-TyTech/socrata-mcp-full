...
export const userSearchTool = {
  ...
  handler: async (args: any, extra: any) => {
    ...
    try {
      ...
      if (idsParam) url.searchParams.append('ids', idsParam);

      const emailsParam = normalizeToCommaSeparated(emails);
      if (emailsParam) url.searchParams.append('emails', emailsParam);

      const rolesParam = normalizeToCommaSeparated(roles);
      if (rolesParam) url.searchParams.append('roles', rolesParam);

      if (disabled !== undefined) url.searchParams.append('disabled', disabled ? 't' : 'f');
      if (future !== undefined) url.searchParams.append('future', future ? 't' : 'f');
...
export const teamSearchTool = {
  ...
  handler: async (args: any, extra: any) => {
    ...
    try {
      ...
      if (namesParam) url.searchParams.append('names', namesParam);
...
export async function getUserRoles(domain: string, userId: string): Promise<any> {
  validateDomain(domain);
  const url = `https://${domain}/api/catalog/v1/users/${userId}/roles`;
  return await socrataGet(url);
}

export async function updateUserRoles(domain: string, userId: string, roles: string[]): Promise<any> {
  validateDomain(domain);
  const url = `https://${domain}/api/catalog/v1/users/${userId}/roles`;
