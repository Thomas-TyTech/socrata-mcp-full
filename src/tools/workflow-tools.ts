...
export async function getWorkflowContext(domain: string, workflowId: string): Promise<any> {
  validateDomain(domain);
  const url = `https://${domain}/api/catalog/v1/workflows/${workflowId}/context`;
  return await socrataGet(url);
}

export async function updateWorkflowContext(domain: string, workflowId: string, context: any): Promise<any> {
  validateDomain(domain);
  const url = `https://${domain}/api/catalog/v1/workflows/${workflowId}/context`;
